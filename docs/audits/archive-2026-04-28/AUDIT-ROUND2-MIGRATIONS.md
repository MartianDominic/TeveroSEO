...
All rollback scripts are located in `open-seo-main/drizzle/rollback/`:

| File | Migration | Priority |
|------|-----------|----------|
| `0002_rollback.sql` | Clerk auth migration (DROP TABLE) | CRITICAL |
| `0021_rollback.sql` | Change tracking tables | STANDARD |
| `0029_rollback.sql` | Client ID type fixes | CRITICAL |
| `0030_rollback.sql` | Race condition constraints | STANDARD |
| `0031_rollback.sql` | FK constraints | CRITICAL |
| `0032_database_schema_improvements_rollback.sql` | Indexes, constraints, enums | HIGH |
| `0032_rename_gsc_snapshots_rollback.sql` | Table rename | HIGH |
| `0033_rollback.sql` | Data integrity constraints | STANDARD |

See `open-seo-main/drizzle/rollback/README.md` for usage instructions.

### Safe Migration Variants Created

These are improved versions of destructive migrations that include data backup:

| File | Purpose |
|------|---------|
| `0002_clerk_auth_migration_safe.sql` | Creates backup tables before DROP TABLE |
| `0029_fix_client_id_types_and_fks_safe.sql` | Adds validation and backup before DELETE |
| `0031_add_missing_fk_constraints_safe.sql` | Creates backup tables for all orphan records, adds threshold check |

Use these instead of originals when data preservation is needed.

### Index Batching

Split `0032_database_schema_improvements.sql` index creation into 3 batches:

| File | Indexes | Purpose |
|------|---------|---------|
| `0032_indexes_batch1.sql` | 10 | link_graph, page_links, orphan_pages |
| `0032_indexes_batch2.sql` | 10 | link_opportunities, link_suggestions, voice |
| `0032_indexes_batch3.sql` | 17 | Remaining FK indexes |

Run batches sequentially during maintenance window. Each batch takes ~1-2 minutes on tables with <100K rows.

### Compatibility Views

Created `0032_rename_gsc_snapshots_with_view.sql` which includes:

```sql
CREATE OR REPLACE VIEW gsc_snapshots AS SELECT * FROM seo_gsc_snapshots;
```

This backward compatibility view allows code referencing the old table name to continue working during rolling deployments. Remove the view after all code is updated:
...
All `*_safe.sql` migrations create backup tables with naming convention:
- `_backup_{table}_XXXX` where XXXX is the migration number

To restore deleted data after rollback:
```sql
INSERT INTO {table} SELECT * FROM _backup_{table}_XXXX;
```

To clean up backup tables after successful migration:
```sql
DROP TABLE IF EXISTS _backup_{table}_XXXX;
```
...
open-seo-main/drizzle/
  rollback/
    README.md                              (NEW)
    0002_rollback.sql                      (NEW)
    0021_rollback.sql                      (NEW)
    0029_rollback.sql                      (NEW)
    0030_rollback.sql                      (NEW)
    0031_rollback.sql                      (NEW)
    0032_database_schema_improvements_rollback.sql (NEW)
    0032_rename_gsc_snapshots_rollback.sql (NEW)
    0033_rollback.sql                      (NEW)
  0002_clerk_auth_migration_safe.sql       (NEW)
  0029_fix_client_id_types_and_fks_safe.sql (NEW)
  0031_add_missing_fk_constraints_safe.sql (NEW)
  0032_indexes_batch1.sql                  (NEW)
  0032_indexes_batch2.sql                  (NEW)
  0032_indexes_batch3.sql                  (NEW)
  0032_rename_gsc_snapshots_with_view.sql  (NEW)
```
