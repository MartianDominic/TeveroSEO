...
registerCheck({
  ...
  runV2: (ctx: SEODataContext): CheckResult => {
    const internal = ctx.data.internal_link_count;
    const passed = internal >= 3 && internal <= 10;
    return {
      checkId: "T1-39",
      passed,
...
registerCheck({
  ...
  runV2: (ctx: SEODataContext): CheckResult => {
    // Check if internal links exist in the first two paragraphs
    const paragraphs = (ctx.data as any).paragraphs ?? [];
    const firstTwoParagraphs = paragraphs.slice(0, 2).join(" ").toLowerCase();
    const internalLinks = ctx.data.internal_links;
    // Check if any internal link anchor text appears in first paragraphs
    const hasLink = internalLinks.some(link =>
      firstTwoParagraphs.includes(link.text.toLowerCase())
    ) || (internalLinks.length > 0 && paragraphs.length === 0);
    // Note: This is a simplified check - the Python extractor doesn't track link positions
    // For accurate detection, would need link position data from extractor
    return {
      checkId: "T1-40",
      passed: hasLink,
      severity: hasLink ? "info" : "medium",
...
registerCheck({
  ...
  run: (ctx: CheckContext): CheckResult => {
    ...
    // Find URLs with duplicate same anchor (unused in original, keep for reference)
    let duplicates = 0;
    // Note: urlAnchors tracking done below with seen map
  ...
  runV2: (ctx: SEODataContext): CheckResult => {
    const links = ctx.data.internal_links;
    // Count links where same URL + same anchor appears multiple times
    const seen = new Map<string, number>();
    for (const link of links) {
...
registerCheck({
  ...
  runV2: (ctx: SEODataContext): CheckResult => {
    // Check internal links for empty anchor text
    const allLinks = [...ctx.data.internal_links, ...ctx.data.external_links];
    let emptyLinks = 0;
    for (const link of allLinks) {
      const text = link.text.trim();
      // Note: JSON extraction doesn't track img children or aria-label
      // This is a simplified check based on text content only
      if (!text) emptyLinks++;
    }
    const passed = emptyLinks === 0;
    return {
