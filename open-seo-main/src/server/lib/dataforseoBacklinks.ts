...
/** Timeout for DataForSEO API requests (30 seconds) */
...
async function createAuthenticatedFetch() {
  ...
  return async (url: RequestInfo, init?: RequestInit): Promise<Response> => {
    ...
    const timeoutId = setTimeout(() => controller.abort(), DATAFORSEO_TIMEOUT_MS);
    ...
    try {
      ...
      if (error instanceof Error && error.name === "AbortError") {
        throw new AppError(
          "INTERNAL_ERROR",
          `DataForSEO request timed out after ${DATAFORSEO_TIMEOUT_MS}ms`
        );
      }
      throw error;
    } finally {
