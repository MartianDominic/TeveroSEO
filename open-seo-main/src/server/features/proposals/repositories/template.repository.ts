...
export async function createTemplate(
  ...
  const insertedSections: TemplateSectionSelect[] = [];
...
export async function updateSectionOrder(
  ...
  // Update position on each section
  for (let i = 0; i < sectionOrder.length; i++) {
    await db
      ...
      .where(
        and(
          eq(templateSections.id, sectionOrder[i]),
...
> {
  ...
  const [newTemplate] = await db
    ...
    .values({
      ...
      isDefault: false,
      isArchived: false,
      sectionOrder: [], // Will update after creating sections
    })
    .returning();

  // Duplicate sections
  const newSections: TemplateSectionSelect[] = [];
  ...
  // Update section order with new IDs
  ...
  await db
    ...
    .where(eq(proposalTemplates.id, newTemplateId));
