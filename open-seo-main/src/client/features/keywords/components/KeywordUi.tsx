...
export function OverviewStats({ keyword }: { keyword: KeywordResearchRow }) {
  return (
    <div className="shrink-0 bg-background border border-border rounded-xl px-4 py-2.5 flex items-center gap-4 min-h-[48px]">
      ...
      <div className="flex items-center gap-4 text-sm flex-wrap min-w-0">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-foreground/50">CPC</span>
          <span className="font-semibold tabular-nums">
            {keyword.cpc == null ? "-" : `$${keyword.cpc.toFixed(2)}`}
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-foreground/50">Comp</span>
          <span className="font-semibold tabular-nums">
...
export function KeywordRow({
  ...
  return (
    <div
      className={`flex items-center gap-3 px-4 py-2 border-b border-border text-sm hover:bg-muted/50 transition-colors cursor-pointer ${
    ...
    >
      ...
      <span className="w-16 text-right tabular-nums text-foreground/70">
        {formatNumber(row.searchVolume)}
      </span>
      <span className="w-14 text-right tabular-nums text-foreground/70">
        {row.cpc == null ? "-" : row.cpc.toFixed(2)}
      </span>
      <span className="w-12 text-right tabular-nums text-foreground/70">
...
export function KeywordCard({
  ...
  return (
    <div
      className={`bg-background border border-border rounded-lg p-3 space-y-2 cursor-pointer transition-colors ${
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 text-xs-safe">
        ...
        <div className="text-center">
          <p className="text-foreground/50">CPC</p>
          <p className="font-medium tabular-nums">
            {row.cpc == null ? "-" : `$${row.cpc.toFixed(2)}`}
          </p>
        </div>
        <div className="text-center">
          <p className="text-foreground/50">Comp.</p>
          <p className="font-medium tabular-nums">
            {row.competition == null ? "-" : row.competition.toFixed(2)}
          </p>
        </div>
