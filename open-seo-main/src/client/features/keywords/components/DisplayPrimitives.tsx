...
export function IntentBadge({ intent }: { intent: KeywordIntent }) {
  const variantMap: Record<KeywordIntent, { className: string }> = {
    informational: { className: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border-blue-200 dark:border-blue-800" },
    commercial: { className: "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800" },
    transactional: { className: "bg-success/20 text-green-700 dark:text-green-400 border-success/30" },
    navigational: { className: "" },
    unknown: { className: "bg-muted text-muted-foreground border-border" },
  };
  ...
  const { className } = variantMap[intent];
