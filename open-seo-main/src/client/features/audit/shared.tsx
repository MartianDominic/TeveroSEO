...
export function StatusBadge({ status }: { status: string }) {
  ...
  if (status === "completed") {
    return (
      <Badge className="bg-success/5 text-green-700 dark:text-green-400 text-xs-safe border-success/30 gap-1">
  ...
  if (status === "cancelled") {
    return (
      <Badge variant="secondary" className="text-xs-safe gap-1">
...
export function HttpStatusBadge({ code }: { code: number | null }) {
  if (!code) return <Badge variant="secondary" className="text-xs-safe">-</Badge>;
  if (code >= 200 && code < 300) {
    return <Badge className="bg-success/20 text-green-700 dark:text-green-400 text-xs-safe border-success/30">{code}</Badge>;
  }
  if (code >= 300 && code < 400) {
    return <Badge variant="outline" className="text-yellow-600 text-xs-safe">{code}</Badge>;
  }
  return <Badge variant="destructive" className="text-xs-safe">{code}</Badge>;
}
