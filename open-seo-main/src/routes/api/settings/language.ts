...
export const Route = createAPIFileRoute("/api/settings/language")({
  /**
  ...
  GET: async ({ request }) => {
    ...
    try {
      ...
      log.error("Failed to get language settings", { error, workspaceId: user.organizationId });
      return json({ success: false, error: "Failed to get language settings" }, { status: 500 });
    }
  },
  ...
  PUT: async ({ request }) => {
    ...
    try {
      ...
      log.error("Failed to update language settings", { error, workspaceId: user.organizationId });
      return json({ success: false, error: "Failed to update language settings" }, { status: 500 });
    }
  },
});
