...
describe("CSV White-Label Branding", () => {
  describe("Header Template", () => {
    it("should use agency brand name in header when configured", () => {
      const brandName = "AcmeAgency";
      const headerTemplate = "# {brand_name} Analytics Export";
      const expectedHeader = headerTemplate.replace("{brand_name}", brandName);

      expect(expectedHeader).toBe("# AcmeAgency Analytics Export");
      expect(expectedHeader).not.toContain("TeveroSEO");
    });

    it("should support {report_type} placeholder in header", () => {
      const brandName = "AcmeAgency";
      const reportType = "Keyword";
      const headerTemplate = "# {brand_name} {report_type} Report";

      const result = headerTemplate
        .replace("{brand_name}", brandName)
        .replace("{report_type}", reportType);

      expect(result).toBe("# AcmeAgency Keyword Report");
    });

    it("should fall back to default header when no template configured", () => {
      const defaultTemplate = "# {brand_name} Analytics Export";
      const brandName = "TeveroSEO";

      const result = defaultTemplate.replace("{brand_name}", brandName);

      expect(result).toBe("# TeveroSEO Analytics Export");
    });

    it("should handle placeholder case-insensitivity", () => {
      const brandName = "TestAgency";
      const templates = [
        "# {brand_name} Report",
        "# {Brand_Name} Report",
        "# {BRAND_NAME} Report",
      ...
      templates.forEach((template) => {
        const result = template.replace(/\{brand_name\}/gi, brandName);
        expect(result).toBe("# TestAgency Report");
      });
    });
  });

  describe("Column Labels", () => {
    it("should use custom column labels when configured", () => {
      const defaultHeaders = ["keyword", "position", "clicks", "impressions"];
      ...
      const brandedHeaders = defaultHeaders.map(
        (header) => customLabels[header] ?? header
      );

      expect(brandedHeaders).toEqual(["Search Term", "Rank", "Visits", "Views"]);
    });

    it("should preserve column order when applying custom labels", () => {
      const headers = ["url", "clicks", "position"];
      const customLabels: Record<string, string> = {
        url: "Landing Page",
        clicks: "User Clicks",
        position: "SERP Rank",
      ...
      const result = headers.map((h) => customLabels[h] ?? h);

      expect(result[0]).toBe("Landing Page");
      expect(result[1]).toBe("User Clicks");
      expect(result[2]).toBe("SERP Rank");
    });

    it("should fall back to original header when no custom label exists", () => {
      const headers = ["url", "clicks", "new_column"];
      const customLabels: Record<string, string> = {
        url: "Landing Page",
        clicks: "User Clicks",
      };
      ...
      expect(result).toEqual(["Landing Page", "User Clicks", "new_column"]);
    });

    it("should handle partial custom labels gracefully", () => {
      const headers = ["keyword", "position", "clicks", "ctr", "impressions"];
      const partialLabels: Record<string, string> = {
        keyword: "Search Query",
        // Other labels not configured - should use defaults
      };

      const result = headers.map((h) => partialLabels[h] ?? h);

      expect(result[0]).toBe("Search Query");
      expect(result[1]).toBe("position");
      expect(result[2]).toBe("clicks");
    });
  });

  describe("Filename Branding", () => {
    it("should include agency name in filename", () => {
      const agencyName = "AcmeAgency";
      const clientName = "TestClient";
      const dateStr = "2024-01-15";

      const filename = `${agencyName}_${clientName}_Analytics_${dateStr}.csv`;

      expect(filename).toBe("AcmeAgency_TestClient_Analytics_2024-01-15.csv");
      expect(filename).toContain(agencyName);
    });

    it("should sanitize agency name in filename", () => {
      const unsafeAgencyNames = [
        "Agency<script>",
        "Agency/Name",
        "Agency\\Name",
        "Agency Name With Spaces",
      ];

      unsafeAgencyNames.forEach((name) => {
        const sanitized = name
          .replace(/[^a-zA-Z0-9-_\s]/g, "")
          .replace(/\s+/g, "-")
          .toLowerCase()
  ...
  describe("Footer Branding", () => {
    it("should include agency name in footer", () => {
      const brandName = "AcmeAgency";
      const footer = `# Generated by ${brandName}`;

      expect(footer).toBe("# Generated by AcmeAgency");
      expect(footer).not.toContain("TeveroSEO");
    });
  });
  ...
  describe("UTF-8 BOM", () => {
    ...
    it("should handle UTF-8 characters in agency names", () => {
      const internationalAgencyNames = [
        "Agence Franaise",
        "Deutsche Agentur",
        "Agencia Espanola",
      ];

      internationalAgencyNames.forEach((name) => {
        const encoded = Buffer.from(name, "utf-8").toString("utf-8");
  ...
  describe("Backward Compatibility", () => {
    it("should use default branding when no custom config exists", () => {
      ...
      const headerTemplate = defaultBranding.csvHeaderTemplate ?? "# {brand_name} Analytics Export";
      const brandName = defaultBranding.companyName;

      const result = headerTemplate.replace("{brand_name}", brandName);

      expect(result).toBe("# TeveroSEO Analytics Export");
    });

    it("should handle null column labels by using defaults", () => {
      const csvColumnLabels: Record<string, string> | null = null;
      ...
      const headers = ["keyword", "position", "clicks"];
      const labels = csvColumnLabels ?? defaultLabels;
      const result = headers.map((h) => labels[h] ?? h);

      expect(result).toEqual(["Keyword", "Position", "Clicks"]);
    });
  });
});
