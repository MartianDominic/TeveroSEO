...
/**
 * CSV white-label branding configuration.
 */
interface CsvBrandingConfig {
  brandName: string;
  headerTemplate: string | null;
  columnLabels: Record<string, string | undefined> | null;
}

/**
 * Default CSV header template.
 */
const DEFAULT_CSV_HEADER_TEMPLATE = "# {brand_name} Analytics Export";

/**
 * Apply column label overrides to headers.
 * Maps internal column names to branded display names.
 */
function applyColumnLabelOverrides(
  headers: string[],
  columnLabels: Record<string, string | undefined> | null
): string[] {
  if (!columnLabels) {
  ...
  return headers.map((header) => {
    ...
    const exactMatch = columnLabels[header];
    if (exactMatch) {
      return exactMatch;
    }
    ...
    for (const [key, value] of Object.entries(columnLabels)) {
      if (value && key.toLowerCase().replace(/[_\s]/g, "") === normalized) {
        return value;
      }
    }
    return header;
...
/**
 * Generate CSV content for multiple sections with white-label branding.
 */
function generateMultiSectionCsv(
  ...
  branding?: CsvBrandingConfig
): string {
  const exportService = getAnalyticsExportService();
  const lines: string[] = [];

  // Determine brand name (agency name or default)
  const brandName = branding?.brandName ?? "TeveroSEO";

  // Build header from template or default
  const headerTemplate = branding?.headerTemplate ?? DEFAULT_CSV_HEADER_TEMPLATE;
  const headerLine = headerTemplate
    .replace(/\{brand_name\}/gi, brandName)
    .replace(/\{report_type\}/gi, "Analytics");

  // Add UTF-8 BOM for Excel compatibility with special characters
  const BOM = "﻿";

  // Add metadata header with white-label branding
  ...
  // Add each section with branded column labels
  for (const section of sections) {
    lines.push(`## ${section.name}`);
    // Apply column label overrides
    const brandedHeaders = applyColumnLabelOverrides(section.headers, branding?.columnLabels ?? null);
    lines.push(brandedHeaders.join(","));
  ...
  // Add footer with agency branding
    ...
    handlers: {
      POST: async ({
        ...
        try {
          ...
          // Step 11b: Get white-label branding for CSV export
          const { getClientBranding } = await import("@/server/services/portal/pdf-export-service");
          const csvBranding = await getClientBranding(clientId);
          ...
          // Step 14: Generate CSV content with white-label branding
          const csv = generateMultiSectionCsv(
            ...
            {
              brandName: csvBranding.companyName,
              headerTemplate: csvBranding.csvHeaderTemplate,
              columnLabels: csvBranding.csvColumnLabels,
            }
          );
          ...
          // Step 16: Build response with proper headers (white-labeled filename)
          const safeAgencyName = sanitizeFilename(csvBranding.companyName);
          const safeClientName = sanitizeFilename(clientName);
          const dateStr = new Date().toISOString().split("T")[0];
          const filename = `${safeAgencyName}_${safeClientName}_Analytics_${dateStr}.csv`;
