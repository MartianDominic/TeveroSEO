...
import { TeveroMark } from '../brand/TeveroLogo';
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
const usePlatformHealth = (): PlatformHealth => {
  ...
  useEffect(() => {
    ...
    apiClient
      .get<Array<{ key_name: string; required: boolean; configured: boolean }>>('/api/platform-secrets/status')
      .then((res) => {
        const data = res.data;
        ...
        const standardConfigured = standardRequired.filter((s) => s.configured).length;
        ...
        const baseCode = data.find((s) => s.key_name === 'dataforseo_base_code');
        const dataforseoOk =
    ...
    return (
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            {name ? clientInitial(name) : <Globe className="h-4 w-4 text-muted-foreground" />}
          </button>
        </PopoverTrigger>
        <PopoverContent
          className="w-64 p-0"
      ...
      <PopoverTrigger asChild>
        ...
        >
          ...
          <span
            ...
            style={name ? { backgroundColor: seedColor(name) } : { backgroundColor: 'hsl(var(--muted))' }}
          >
...
export const AppShell: React.FC<AppShellProps> = ({ children }) => {
  ...
  const isActive = useCallback(
    ...
    [location.pathname, activeClientId]
  );
