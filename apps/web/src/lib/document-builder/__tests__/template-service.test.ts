...
describe("template-service", () => {
  ...
  describe("validateFrameworkCompliance", () => {
    const createMockBlock = (type: string): PersuasionBlock => ({
      id: `block-${type}`,
    ...
    it("returns valid for no framework", () => {
      ...
      const result = validateFrameworkCompliance(blocks, "");
    ...
    it("returns valid when all required blocks present", () => {
      ...
      const result = validateFrameworkCompliance(blocks, "pas");
      expect(result.isValid).toBe(true);
  ...
  describe("getFrameworkSequence", () => {
    it("returns sequence for valid framework", () => {
      const sequence = getFrameworkSequence("pas");
      expect(sequence).toContain("pain_amplifier");
      expect(sequence).toContain("cta");
    });

    it("returns empty array for unknown framework", () => {
      const sequence = getFrameworkSequence("unknown");
