...
/**
 * SEO Chat Page Client Component
 * Phase 98-08: Client-side rendering for SEO Chat landing page
 *
...
type AnyLinkHref = Parameters<typeof Link>[0]['href'];
type AnyRoute = Parameters<ReturnType<typeof useRouter>['push']>[0];

// ---------------------------------------------------------------------------
// Main Client Component
// ---------------------------------------------------------------------------

interface SeoChatPageClientProps {
  workspaceId: string;
}

export function SeoChatPageClient({ workspaceId }: SeoChatPageClientProps) {
  return (
      ...
      <aside className="w-80 border-r bg-muted/30 flex flex-col">
        <div className="p-4 border-b">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href={'/seo-chat/settings' as AnyLinkHref}>
...
function NewSessionButton({ workspaceId, variant = 'outline' }: NewSessionButtonProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      router.push(`/seo-chat/${sessionId}` as AnyRoute);
      setOpen(false);
    } catch (err) {
